...
//Addition by Jianyang for ZLE
int zle_thr_file[MAX_CH] = { 0 };
...
int ParseConfigFile(FILE *f_ini, WaveDumpConfig_t *WDcfg) 
{
	...
	while(!feof(f_ini)) {
		...
		/************MODIFICATION BY JIANYANG************/

        // Coincidence Level
	    if (strstr(str, "COINCIDENCE")!=NULL) {
			read = fscanf(f_ini, "%d", &WDcfg->CoincidenceLevel);
			continue;
		}
		...
		/*************** ENABLING OF ZLE ***************/
        // Zero-length encoding enabling!
		if (strstr(str, "ZLE_ENABLED")!=NULL) {
			read = fscanf(f_ini, "%s", str1);
			if (strcmp(str1, "DISABLED")==0)
                WDcfg->ZLEOn = CAEN_DGTZ_ZS_NO;
			else if (strcmp(str1, "ENABLED")==0)
                WDcfg->ZLEOn = CAEN_DGTZ_ZS_ZLE;
            else
	...
	// LVDS Settings
	        if (strstr(str, "LVDS")!=NULL){
			read = fscanf(f_ini, "%s", str1);
			if (strcmp(str1, "VETO")==0)
		    WDcfg->LVDSVeto = 2;
		else
		    printf("%s: Invalid Parameter (currently only accepts VETO)\n", str);
		continue;
		}
...
//				WDcfg->FTThreshold[tr] = val;
		...
		/******************* MODIFICATION BY JIANYANG *******************/
		// ZLE Threshold
		if (strstr(str, "ZLE_THRESHOLD")!=NULL) {
			read = fscanf(f_ini, "%d", &val);
            if (ch == -1)
				for (i = 0; i < MAX_SET; i++)
				{
					WDcfg->ZLEThreshold[i] = val;
					zle_thr_file[i] = val;
				}
			else
			{
				WDcfg->ZLEThreshold[ch] = val;
				zle_thr_file[ch] = val;
			}
			continue;
		}
