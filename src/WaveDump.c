...
char *make_path(char *c_t, char *op){
	...
	sprintf(path, "%ssandix_%s/", op, c_t);
	return path;
}
...
int ProgramDigitizer(int handle, WaveDumpConfig_t WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)
{
    ...
    /******************ADDITION BY JIANYANG******************/
    uint32_t zs_threshold_register_data, zs_sample_register_data;
    if (WDcfg.ZLEOn==CAEN_DGTZ_ZS_ZLE){
    ...
    if ((BoardInfo.FamilyCode == CAEN_DGTZ_XX740_FAMILY_CODE) || (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE)){
        ...
        for (i = 0; i < WDcfg.Nch; i++) {
            if (WDcfg.EnableMask & (1<<i)) {
                ...
                /******************ADDITION BY JIANYANG******************/
                if (WDcfg.ZLEOn == CAEN_DGTZ_ZS_ZLE) {
                    //Program the zle threshold
                    ret |= CAEN_DGTZ_ReadRegister(handle, (0x1024+ (i<<8)), &zs_threshold_register_data);
                    // ret |= CAEN_DGTZ_WriteRegister(handle, (0x1024 + (i<<8)), ((zs_threshold_register_data&0xFFFFF000)|WDcfg.ZLEThreshold[i]));
                    // ret |= CAEN_DGTZ_WriteRegister(handle, (0x1024 + (i<<8)), ((zs_threshold_register_data&((((uint32_t) 1)<<31)-1))|(WDcfg.PulsePolarity[i]<<31)));
                    ret |= CAEN_DGTZ_WriteRegister(handle, (0x1024 + (i<<8)), (WDcfg.PulsePolarity[i]<<31)+WDcfg.ZLEThreshold[i]);

                    //Program the ZLE lookforwards/backwards samples
                    ret |= CAEN_DGTZ_ReadRegister(handle, (0x1028 + (i<<8)), &zs_sample_register_data);
                    ret |= CAEN_DGTZ_WriteRegister(handle, (0x1028 + (i<<8)), WDcfg.ZLELFW[i] + (WDcfg.ZLELBK[i]<<16));
                }
                /*******************************************************/
            }
        }
    ...
    ret |= CAEN_DGTZ_WriteRegister(handle, 0x810C, (global_trigger_reg_data|(WDcfg.CoincidenceLevel<<24)));

    
    if (WDcfg.Veto == 2) {
        printf("Using external trigger as a veto\n");
            //Program the global trigger
        ret |= CAEN_DGTZ_WriteRegister(handle, 0x810C, (global_trigger_reg_data|((1<<27)+(0xF<<20)))); // Sets the external trigger to AND
        ret |= CAEN_DGTZ_ReadRegister(handle, 0x810C, &global_trigger_reg_data);


        //Program the FPIO
        ret |= CAEN_DGTZ_ReadRegister(handle, 0x811C, &fpio_reg_data);
        ret |= CAEN_DGTZ_WriteRegister(handle, 0x811C, (fpio_reg_data|(1<<10))&(0xFFFFF7FF));
        ret |= CAEN_DGTZ_ReadRegister(handle, 0x811C, &fpio_reg_data);
    }
...
void Calibrate_XX740_DC_Offset(int handle, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo){
	...
	#ifdef _WIN32Event16->DataChannel
				Sleep(200);
	#else
				usleep(200000);
	#endif
...
*   \param   Event Pointer to the Event to write
*/
int WriteOutputFiles(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event, char *c_t, char* fp)
{
    int ch, j, ns;
    CAEN_DGTZ_UINT16_EVENT_t  *Event16 = NULL;
    CAEN_DGTZ_UINT8_EVENT_t   *Event8 = NULL;
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++) {
        ...
        if( WDcfg->OutFileFlags& OFF_BINARY) {
            ...
            if (!WDrun->fout[ch]) {
                char fname[100];
                sprintf(fname, "%sSanDix_Wave_%s_%d.dat", fp, c_t, ch);
 ...
 * 
 */
 int MergeOutputFiles(FILE *fname)
{
    int ch, j, ns;
    ...
    for (ch = 0; ch < 8; ch++) {
		...
		char ffname[100];
		sprintf(ffname, "wave_ch%d.dat",ch);
		printf(ffname);
		printf("\n");
		FILE *fp1 = fopen(ffname,"rb");

		char c;
		if (fp1 == NULL || fname == NULL)
		{
			printf("Failed to load/read files\n");
			exit(0);
		}
...
/*! Write Header ONLY to a file named [fname] 
 *
 * 
 * 
*/
int WriteOutputHeader(FILE *fname)
{
	uint32_t BinHeader[6];
	//BinHeader[0] = "?";//(WDcfg->Nbit == 8) ? Size + 5*sizeof(*BinHeader) : Size*2 + 5*sizeof(*BinHeader);
	//BinHeader[1] = "?";//EventInfo->BoardId;
	//BinHeader[2] = "?";//EventInfo->Pattern;
	//BinHeader[3] = "?";//EventInfo->EventCounter;
	//BinHeader[4] = "?";//EventInfo->TriggerTimeTag;
	
	if(fwrite(BinHeader, sizeof(*BinHeader), 5, fname) != 5) {
...
void TurnOnDataTaking(int handle, WaveDumpRun_t *WDrun, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)
{
	if (BoardInfo.FamilyCode != CAEN_DGTZ_XX742_FAMILY_CODE)//XX742 not considered
		Set_relative_Threshold(handle, WDcfg, BoardInfo);
...
int main(int argc, char *argv[])
{
    ...
    //time(&tm);
    char* c_t = time_stamp();
 /*   char* FP = make_path(c_t, OutputSavePath);
    //printf("\n");
    //printf(FP);
    struct stat st = {0};
	...
	if (stat(FP, &st) == -1) {
		...
		printf("\n");
		printf("---------------------------------");
		printf("Successfully created folder %s",FP);
		printf("---------------------------------");
		printf("\n");
	}
*/
...
Restart:
    ...
    uint32_t zs_thresh_reg, zs_samp_reg;
    for (int i = 0; i < 8; i++){
        if (WDcfg.ZLEOn == CAEN_DGTZ_ZS_ZLE) {
            //Program the zle threshold
            ret |= CAEN_DGTZ_ReadRegister(handle, (0x1024+ (i<<8)), &zs_thresh_reg);
            //Program the ZLE lookforwards/backwards samples
            ret |= CAEN_DGTZ_ReadRegister(handle, (0x1028 + (i<<8)), &zs_samp_reg);

            printf("Channel %i ZLE Threshold Register: %ui\n", i, zs_thresh_reg);
            printf("Channel %i ZLE Sample Register: %i\n", i, zs_samp_reg);
        }
    }   
...
// InterruptTimeout:
        ...
        Ne += NumEvents;
        // if (NumEvents>0)
        //     printf("%i events have been read, total buffer size is %i\n", NumEvents, BufferSize);
        ...
        // for(i = 0; i < (int)NumEvents; i++) {
            ...
            //     }
                ...
                //         ret = WriteOutputFiles(&WDcfg, &WDrun, &EventInfo, Event8, c_t, OutputSavePath);
                //     }
                //     else {
				// 		if (NumEventsSaved>=MaxEventsSaved){
				// 			break;
                ...
                //         ret = WriteOutputFiles(&WDcfg, &WDrun, &EventInfo, Event16, c_t, OutputSavePath);
                //         NumEventsSaved+=1;
